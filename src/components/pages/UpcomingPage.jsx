...
const UpcomingPage = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Filter upcoming tasks (due after today, not completed)
  ...
  const handleAddTask = async (taskData) => {
    try {
      ...
      if (!newTask.completed && newTask.dueDate) {
        ...
        if (isAfter(dueDate, startOfDay(new Date()))) {
  ...
  const handleSaveTask = async (taskData) => {
    try {
      if (editingTask) {
        ...
        // Check if task still belongs to upcoming view
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          <ApperIcon name="CalendarDays" className="h-8 w-8 text-primary" />
          <h1 className="text-3xl font-bold font-display text-gray-900">Upcoming</h1>
